...
local function handle_streamed_error(data)
  ...
  if json.error.message then
    ...
    return {
      status = "error",
      output = "Deepseek Adapter - " .. json.error.message,
    }
  end
end
...
return {
  ...
  callbacks = {
    ...
    inline_output = function(data, context)
      if data and data ~= "" then
        ...
        --- Some third-party Deepseek forwarding services may have a return package with an empty json.choices.
        if #json.choices == 0 then
          return
        end
  ...
  schema = {
    ...
    logit_bias = {
      ...
      optional = true,
      default = nil,
      desc = "Modify the likelihood of specified tokens appearing in the completion. Maps tokens (specified by their token ID) to an associated bias value from -100 to 100. Use https://platform.Deepseek.com/tokenizer to find token IDs.",
      subtype_key = {
        type = "integer",
      },
      subtype = {
    ...
    user = {
      ...
      optional = true,
      default = nil,
      desc = "A unique identifier representing your end-user, which can help Deepseek to monitor and detect abuse. Learn more.",
      validate = function(u)
